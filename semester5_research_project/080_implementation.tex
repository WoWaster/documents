...
\remark{Требуется починка moc, при этом требуется менять CMakeLists.txt либо запускать moc из sysroot под QEMU}

\subsection{Trik Runtime (QMake)}

\remark{Тут должен быть рассказ про нативную сборку в эмуляторе двумя способами: в Docker контейнере и chroot}

\subsection{Umockdev (Meson)}

\remark{Тут должен быть рассказ про использование Meson с Vala, с примером тулчейн файла и personality файла для pkgconf}

\subsection{Trik Runtime (Jenkins)}

\remark{Тут дожен быть рассказ про настройку Jenkins на VisionFive2, про пересборку ядра с поддержкой Docker, куда-то стоит залить конфиг ядра.}
